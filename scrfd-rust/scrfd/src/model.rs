...
impl Scrfd {
    ...
    pub fn from_bytes(model_bytes: &[u8]) -> TractResult<Self> {
        let model = tract_onnx::onnx()
            .model_for_read(&mut io::Cursor::new(model_bytes))?
            .into_optimized()?
            .into_runnable()?;
...
impl Scrfd {
    ...
    pub fn detect_with_threshold(
        ...
        ensure!(0.0 <= threshold.score && threshold.score <= 1.0);
        ensure!(0.0 <= threshold.iou && threshold.iou <= 1.0);
        let (scale, blob) = preprocess(img)?;
        let raw_faces = self.forward(blob, threshold.score)?;
        Ok(postprocess(raw_faces, scale, threshold))
    }
    ...
    fn forward(&self, blob: Array4<f32>, score_threshold: f32) -> TractResult<RawFaces> {
        ...
        let mut faces_batches = Vec::with_capacity(self.options.strides.len());
        for (idx, &stride) in self.options.strides.iter().enumerate() {
            ...
            ensure!(scores.shape() == [n, 1] || scores.shape() == [1, n, 1]);
            ensure!(boxes.shape() == [n, 4] || boxes.shape() == [1, n, 4]);
            ensure!(keypoints.shape() == [n, 2 * KPS] || keypoints.shape() == [1, n, 2 * KPS]);
            ...
            faces_batches.push(RawFaces {
                scores,
                boxes,
                keypoints,
            });
        }
        let scores = faces_batches
            .iter()
            .map(|faces| faces.scores.view())
            .collect::<Vec<_>>();
        let boxes = faces_batches
            .iter()
            .map(|faces| faces.boxes.view())
            .collect::<Vec<_>>();
        let keypoints = faces_batches
            .iter()
            .map(|faces| faces.keypoints.view())
            .collect::<Vec<_>>();
        return Ok(RawFaces {
        ...
        fn distance2kps(points: &Array2<f32>, distance: &Array2<f32>) -> Array2<f32> {
            ...
            for i in (0..2 * KPS).step_by(2) {
                let px = &points.column(0) + &distance.column(i);
                let py = &points.column(1) + &distance.column(i + 1);
                kps.push(px);
                kps.push(py);
            }
...
// Boxes must be sorted by score from highest to lowest.
fn nms(boxes: &Array2<f32>, threshold: f32) -> Vec<usize> {
    ...
    while let Some(&i) = order.first() {
        ...
        let union = areas[i] + areas.take(order_tail) - &intersection;

        order = (intersection / union)
            .iter()
            .enumerate()
...
fn postprocess(
    ...
    debug_assert_eq!(
        reversed_argsort(scores.as_slice().unwrap()),
        (0..scores.len()).collect::<Vec<_>>()
    );
    let keep = nms(&boxes, threshold.iou);
...
impl AnchorOptions {
    fn create(self) -> Array2<f32> {
        ...
        let mut buf = Vec::with_capacity(num_of_points * 2);
